...
import {
  Box, Typography, Container, Grid, TextField, Button, Stack, Snackbar, Alert,
      ...
      <Box sx={{ py: 10, position: 'relative', zIndex: 1 }}>
        <Container maxWidth="lg">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
            <Typography variant="overline" sx={{ color: 'accent.main', letterSpacing: 3 }}>IT SERVICES</Typography>
            <Typography variant="h2" sx={{ mt: 1, mb: 2 }}>
              Premium <Box component="span" className="gradient-text">Web Solutions</Box>
            </Typography>
            <Typography variant="body1" sx={{ color: 'text.secondary', maxWidth: 600, mb: 6 }}>
              From stunning business websites to complex web applications, we deliver
              world-class digital solutions tailored to your needs.
            </Typography>
          </motion.div>
          ...
          <Typography variant="h3" sx={{ textAlign: 'center', mb: 6 }}>
            Transparent <Box component="span" className="gradient-text">Pricing</Box>
          </Typography>
          <Grid container spacing={3} sx={{ mb: 10 }}>
          ...
          <Typography variant="h3" sx={{ textAlign: 'center', mb: 4 }}>
            Project <Box component="span" className="gradient-text">Gallery</Box>
          </Typography>
            ...
            {GALLERY.map((img, i) => (
              <ImageListItem
                ...
                sx={{
                  ...
                  '& img': { transition: 'transform 0.6s ease', borderRadius: '16px' },
                  '&:hover img': { transform: 'scale(1.08)' },
                }}
              >
                <Box component="img" src={img} alt={`Project ${i + 1}`} loading="lazy" sx={{ borderRadius: '16px' }} />
              </ImageListItem>
            ))}
          </ImageList>
          ...
          <Box
            sx={{
              maxWidth: 600, mx: 'auto', p: 4, borderRadius: '20px',
              background: '#111111', border: '1px solid rgba(255,255,255,0.08)',
            }}
          >
            <Typography variant="h4" sx={{ textAlign: 'center', mb: 3 }}>Get a Free Quote</Typography>
            <Stack spacing={2}>
              ...
              <TextField label="Service Required" fullWidth value={form.service} onChange={(e) => setForm({ ...form, service: e.target.value })} />
              <TextField label="Message" fullWidth multiline rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} />
              <Button variant="contained" size="large" onClick={handleSubmit} sx={{ background: 'linear-gradient(135deg, #FFD700, #F97316)', color: '#050505' }}>
                Request Quote
              </Button>
            </Stack>
          </Box>
