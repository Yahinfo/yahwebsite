import { Box, Typography, Card, CardContent, Chip, Rating, Button, Stack } from '@mui/material';
...
export default function CourseCard({ course, index = 0 }: CourseCardProps) {
  const { ref, handleMouseMove, handleMouseLeave, handleMouseEnter } = useTilt(8);
    ...
    >
      ...
      >
        <CardContent sx={{ p: 3 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
            <Box
              sx={{
                ...
                color: course.color, fontSize: '1.5rem',
              }}
            >
              <Icon />
            </Box>
          ...
          <Typography variant="h6" sx={{ mb: 1, fontWeight: 600 }}>{course.title}</Typography>
          ...
          <Button
            ...
            sx={{
              ...
              borderRadius: '14px',
              py: 1.2,
              '&:hover': { background: `${course.color}10`, borderColor: course.color },
            }}
            variant="outlined"
          >
            Read More
